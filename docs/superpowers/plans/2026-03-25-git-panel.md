...
```swift
    ...
    func workingDiff(path: String, staged: Bool) throws -> GitFileDiff {
        ...
        if staged {
            ...
            // 注意：直接调用 git_diff_tree_to_index（不需要 placeholder 行）
...
```swift
    ...
    func unstage(paths: [String]) throws {
        ...
        // hc（git_commit*）本身就是 git_object*，Swift 中 OpaquePointer 可直接传入
        // 不需要再做第二次 lookup，直接用 hc 即可
        for path in paths {
            ...
            git_reset_default(r, hc, &pathStrArray)  // hc 即 git_object*
        }
    }
...
⚠️ `TerminalSplitLeafContainer` 改用 `@EnvironmentObject var gitPanelVM` 后，`environmentObject` 的注入在 Task 18 Step 5 才完成。因此 **Task 17 和 Task 18 的编译验证合并到 Task 18 完成后统一执行**，不要在 Task 17 Step 3 单独 Cmd+B。
...
var gitRepo: GitRepository? = nil   // 由外部（PolterttyRootView）传入
@State private var showFileHistorySheet = false
@State private var fileHistoryPath: String?
```

`FileBrowserPanel` 通过 `var gitRepo: GitRepository?` 参数从 `PolterttyRootView` 接收共享的 repo 实例（Task 19 Step 4 传入）。不需要读 EnvironmentObject，直接作为参数传入即可。
...
```bash
grep -rn "keyEquivalent\|NSMenuItem" macos/Sources/App/macOS/ | head -20
# 找到现有菜单注册位置（通常在 AppDelegate.swift 或 MainMenu 相关文件）
```
